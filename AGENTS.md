...
- `README.md` introduces the product to end users. Keep installation commands,
  infrastructure details, and implementation internals out of it.
- `TECHNICAL.md` holds the technical stack, setup, architecture, API/MCP contracts,
  configuration, deployment, and troubleshooting. Keep its optional-feature
  guidance aligned with the code actually generated.
- `AGENTS.md` is the concise coding workflow, project map, and implementation
  rulebook. Link to TECHNICAL.md instead of duplicating its full reference.
- `CONTRIBUTING.md` defines contributor expectations and review/merge requirements.
- Update the README's product description and repository About website link as
  the product evolves. Describe only shipped user-facing features.
...
- `apps/pages/content/docs/` - public, app-usage Markdown documentation content.
  Every docs page, including API/MCP guides, stays accessible without login or
  payment. Product/API authorization remains separate from documentation access.
- `apps/mcp_server/` - hosted MCP tools for agent integrations.
- `apps/core/agents/` - PydanticAI model helpers and agent code.
- `apps/search/` - authenticated Qdrant client and search integration boundary.
- `built_with_bend/settings.py` - environment-driven Django
  settings.
- `built_with_bend/test_settings.py` - pytest-only settings
  that keep cache, media, email, and Django Q2 state local to test runs.
- `built_with_bend/urls.py` - top-level URL routing.
...
1. Read `CONTRIBUTING.md`, relevant sections of `TECHNICAL.md`, `DESIGN.md`,
   `docs/quality.md`, and the files around the
   requested change.
...
COMPOSE_PROJECT_NAME=built_with_bend-agent-a LOCAL_WEB_PORT=8002 LOCAL_POSTGRES_PORT=55434 LOCAL_REDIS_PORT=56381 LOCAL_MAILHOG_SMTP_PORT=11025 LOCAL_MAILHOG_UI_PORT=18025 make serve
```
...
## Directory-specific production contract

- Hosted Djass job 34 regenerated all feature flags as yes except DigitalOcean.
- Public visitors never need accounts. ALLOW_SIGNUPS defaults false; no first-user promotion.
