# Technical guide — Built with Bend

Implementation, local development, configuration, and operations reference.
For product users, see [README.md](README.md); for contribution and agent
workflow, see [CONTRIBUTING.md](CONTRIBUTING.md) and [AGENTS.md](AGENTS.md).
...
With `DEBUG=False`, the root URL configuration uses
`apps.pages.error_views.page_not_found` to render the standalone `404.html`
without request context processors. Keep this template independent of database
queries, shared page layouts, analytics and product schema. It preserves Django
trailing-slash redirects even if a promotional-banner query is unavailable.
Customize its generated project name, colors and support email as needed.
...
- Authenticated Qdrant client configuration with lazy connection setup.
...
- Public product documentation pages with copyable code blocks.
...
```bash
COMPOSE_PROJECT_NAME=built_with_bend-agent-a make agent-services
LOCAL_INSTANCE_ID=built_with_bend-agent-a CACHE_KEY_PREFIX=built_with_bend-agent-a Q_CLUSTER_NAME=built_with_bend-agent-a-q make terminal-worker
```
...
|   `-- pages/                        # Landing, legal, pricing and repo-tracked content
|       `-- content/docs/             # Public app-usage documentation
|-- docs/maintainers/                 # Repository-only development/deployment guides
|-- deployment/
...
| `apps/pages` | Marketing pages, app home, settings, legal pages, pricing, public blog posts, and public Markdown docs content. |
...
| `SERVICE_NAME` | Service facet; defaults to `built_with_bend-web` or `built_with_bend-worker`. |
...
| `QDRANT_URL`, `QDRANT_API_KEY`, `QDRANT_TIMEOUT_SECONDS` | Authenticated Qdrant connection and request timeout. |
...
| `POSTHOG_API_KEY`, `POSTHOG_HOST`, `POSTHOG_BROWSER_HOST`, `POSTHOG_LOGS_ENABLED`, `POSTHOG_LOG_LEVEL` | Automatic browser and server analytics without an opt-in banner plus optional batched privacy-filtered OTLP log export. Use a first-party browser proxy in production when possible. |
...
The repository-only [maintainer guides](docs/maintainers/README.md) include the full
[environment variable reference](docs/maintainers/deployment/environment-variables.md).
...
curl "$SITE_URL/api/user" \
  -H "Authorization: Bearer $BUILT_WITH_BEND_API_KEY"

curl "$SITE_URL/api/user" \
  -H "X-API-Key: $BUILT_WITH_BEND_API_KEY"
```
...
```text
Add Built with Bend MCP support to this repo.

Use MCP URL: <production-url>/mcp/
Use the MCP client's OAuth flow first. If OAuth is unavailable, use the user's Built with Bend API key from an environment variable and send it as X-API-Key or Authorization: Bearer. Do not hardcode, log, or commit any token or key.
...
`gunicorn built_with_bend.asgi:application` with
...
- Keyboard shortcuts are wired from rendered navigation controls without
  adding visual keycap chrome.
...
current template. Add `data-shortcut-key` to new navigation actions only when
the shortcut is stable and unlikely to conflict with text entry.
...
The generated `.github/workflows/reviewgate.yml` is included by default
(`use_reviewgate=y`) independently of application CI and product AI. To activate
reviews, add `OPENROUTER_API_KEY` in the generated repository's GitHub Actions
secrets. An app's `.env` value does not configure the workflow.

A credential-check job checks only whether the key is nonempty; it never checks
out code or logs the key. Without it, review and rereview jobs are skipped and
no model call or ReviewGate check is produced. The check job still appears in
Actions with an explicit skip explanation. A skipped review is not a 5/5 pass.

With a key, eligible same-repository pull requests are reviewed when opened,
updated, reopened, or marked ready. Fork and Dependabot PRs are excluded.
The workflow checks out the exact PR head, tracks `LVTD-LLC/reviewgate@main`,
uses the built-in review model and general/adversarial angles, and allows
300 seconds per angle / 660 seconds total inside a 20-minute job. The runtime
archive's build attestation is verified before execution.

Maintainers can comment exactly `@reviewgate review` after a run finishes.
ReviewGate verifies current write permission and the eligible current-head run;
the rereview job does not check out PR code or receive the OpenRouter key.
Comment triggers work after the workflow reaches the default branch.

Once configured, require the dedicated `ReviewGate` check to succeed with a
completed **5/5** result on the exact current PR head before merging. Provider
errors, incomplete angles, and stale results are not approvals. See
[CONTRIBUTING.md](CONTRIBUTING.md#reviewgate-merge-policy) for the optional-setup
policy and fork contribution path. Branch protection is configured separately;
do not require a check that cannot run before adding the key. See the
[ReviewGate setup reference](https://reviewgate.lvtd.dev/docs/github-actions/).
...
Pytest uses `built_with_bend.test_settings`, which keeps cache,
...
```bash
docker build -f deployment/Dockerfile -t built_with_bend:latest .
```
...
```bash
docker run --env-file .env -e APP_PROCESS_TYPE=server -p 8000:80 built_with_bend:latest
docker run --env-file .env -e APP_PROCESS_TYPE=worker built_with_bend:latest
```
...
cp .env.example .env
docker build -f deployment/Dockerfile -t built_with_bend:latest .
docker compose -f docker-compose-prod.yml -p "built_with_bend" up --detach --remove-orphans
```
...
```bash
fly auth login
fly apps create built-with-bend
...
- Read the repository-only [Fly deployment guide](docs/maintainers/deployment/fly-deployment.md)
  for the full checklist.

### IndexNow

Once the production sitemap is reachable, set the GitHub repository variable
`SITE_URL` to the same canonical HTTPS origin configured in the app's `SITE_URL`
(no path). GitHub Actions needs its own copy of this value because it does not
read the deployed app's environment; no IndexNow-specific URL variable is needed.
The CapRover deploy workflow then automatically notifies IndexNow after the live
revision matches. Leaving the variable unset skips notifications, including on
first deployment. The post-generation hook creates a unique public verification key in
`indexnow-key.txt`; commit this file with the generated project. The app serves it
at `/indexnow-key.txt` automatically. No additional secret or manual key setup is
needed, and changing `SECRET_KEY` or `SITE_URL` does not change the key.
Only current sitemap URLs plus the prior public snapshot are submitted, with
per-batch deduplication, bounded retries and visible failures. Unchanged sitemap
URLs are resubmitted too; database-only updates need a separate trigger.
`python manage.py submit_indexnow --dry-run` is the manual check (omit `--dry-run`
to submit). For other hosts, run this command after deploying. HTTP 202 means
key validation is pending, not confirmed indexing. A failed notification does
not roll back the application; use the original run's snapshot artifact to retry
removed URLs as shown beside the workflow step.

### CapRover

The Python package slug is `built_with_bend`. The CapRover app slug is separate and the generated deploy workflow sets `CAPROVER_APP_NAME=built-with-bend`.

Create four CapRover apps:

- `built-with-bend`
- `built-with-bend-workers`
- `built-with-bend-postgres`
- `built-with-bend-redis`

Runtime app settings:

- Set `APP_PROCESS_TYPE=server` on `built-with-bend`.
- Set `APP_PROCESS_TYPE=worker` on `built-with-bend-workers`.
...
uv run --no-sync gunicorn built_with_bend.asgi:application --bind 0.0.0.0:80 --workers 3 --worker-class uvicorn_worker.UvicornWorker
...
Follow [CONTRIBUTING.md](CONTRIBUTING.md), including its review and merge policy.
...
The generated `package.json` declares MIT. Confirm the intended repository
license and add an appropriate LICENSE file before publishing; package metadata
alone is not a complete licensing policy.
