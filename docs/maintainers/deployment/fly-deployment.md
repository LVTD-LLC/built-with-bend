---
title: Deploying Built with Bend to Fly.io
description: Learn how to deploy Built with Bend on Fly.io.
keywords: Built with Bend, deployment, fly.io, fly, django
author: LVTD, LLC
---
...
- Choose an app name. Fly app names are globally unique and must use DNS-safe characters. The generated config uses `built-with-bend` by default.
...
```bash
fly postgres create --name built-with-bend-db
fly postgres attach built-with-bend-db --app built-with-bend
```
...
```bash
fly redis create --name built-with-bend-redis
fly redis status built-with-bend-redis
...
- `SITE_URL` must match the public HTTPS URL for the app, for example `https://built-with-bend.fly.dev`.
