...
- Document only shipped user-facing features. Keep deployment, development, and
  internal operations in repository-only `docs/maintainers/`, never website docs.
- All docs, including API and optional MCP setup guides, must work without login
  or a paid subscription. Keep docs public and indexable even when the described
  product feature requires authentication or payment. Never interpolate real user data.
- Write for Built with Bend users first, not for maintainers.
