...
def enable_signup_for_scaffold_auth_tests(settings):
    # These tests exercise the retained optional auth scaffold, not public production signup.
...
def test_signup_redirects_to_email_code_verification(client, monkeypatch, settings):
    ...
    monkeypatch.setattr(
        "built_with_bend.adapters.CustomAccountAdapter.send_confirmation_mail",
...
def test_dashboard_does_not_show_email_confirmation_reminder(client):
    ...
    assert "Welcome to Built with Bend" in content
...
def test_mailgun_sender_defaults_are_configurable():
    expected_server_email = "Built with Bend Errors <error@built_with_bend.app>"

    assert (
        settings.DEFAULT_FROM_EMAIL == "LVTD, LLC from Built with Bend <hello@built_with_bend.app>"
    )
    assert settings.SERVER_EMAIL == expected_server_email
    assert settings.ANYMAIL["MAILGUN_SENDER_DOMAIN"] == "mg.built_with_bend.app"
