...
class SignupTrackingMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        if not settings.ALLOW_SIGNUPS:
            raise Http404("Public signup is closed.")
...
class PricingView(TemplateView):
    ...
    def get_template_names(self):
        if not settings.STRIPE_SECRET_KEY:
            return ["directory/sponsorship.html"]
...
def get_category_title(category_slug):
    if category_slug == "getting-started":
...
def docs_page_view(request, category, page):
    """
    Render public documentation using only site-level example context.
    """
    ...
    try:
        ...
        docs_template_context = {
            ...
            "api_key_env_var": "BUILT_WITH_BEND_API_KEY",
